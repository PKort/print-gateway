...
async function refresh() {
  try {
    ...
    document.querySelector('#power-state').textContent = data.power.connected ? (powerLabels[powerState] || powerState) : 'Brak połączenia z HA';
    document.querySelector('#power-on').disabled = !data.power.connected || powerState === 'on';
    document.querySelector('#power-off').disabled = !data.power.connected || powerState === 'off';
...
form.addEventListener('submit', async e => {
  ...
  try {
    ...
    message.className='message success'; message.textContent=`${data.message}${data.job ? ` Numer: ${data.job}.` : ''}`; form.reset(); fileLabel.textContent='Wybierz PDF lub przeciągnij go tutaj'; await refresh();
...
document.addEventListener('click', async e => {
  ...
  await fetch(`/api/jobs/${e.target.dataset.job}/cancel`, {method:'POST', headers:{'X-CSRF-Token':window.GATEWAY.csrf}}); refresh();
});
...
async function setPower(action) {
  ...
  try {
    const response = await fetch('/api/power', {method:'POST', headers:{'Content-Type':'application/json','X-CSRF-Token':window.GATEWAY.csrf}, body:JSON.stringify({action})});
